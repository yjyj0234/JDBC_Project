...
public class CarSelect extends JFrame{
	...
	public CarSelect(String title) {
		...
		//위치 너비
		this.setBounds(100, 100, 1300, 500);
	...
	public void addData() {
		...
		model.setRowCount(0);
			...
			String sql="select c.car_no num, car_model, car_color,car_option,"
					+ "to_char(car_price,'l999,999,999') car_price,order_no, order_name, order_addr,order_hp,"
					+ "order_nationality from orderer o,car c where o.car_no=c.car_no order by c.car_no";
			try {
				...
				while(rs.next()) {
					...
					data.add(rs.getString("car_price"));
